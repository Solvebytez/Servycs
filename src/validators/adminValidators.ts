...
export const approveVendorValidation = [
  param("vendorId").isUUID().withMessage("Invalid vendor ID format"),
...
export const updateSupportTicketValidation = [
  param("ticketId").isUUID().withMessage("Invalid ticket ID format"),
  body("status")
    .optional()
  ...
  body("priority")
    ...
    .isIn(["LOW", "MEDIUM", "HIGH", "URGENT"])
    .withMessage("Invalid priority level"),
  body("assignedTo").optional().isUUID().withMessage("Invalid admin ID format"),
];
...
export const flagContentValidation = [
  ...
  body("contentId").isUUID().withMessage("Invalid content ID format"),
  body("reason")
    .trim()
    .isLength({ min: 10, max: 200 })
    .withMessage("Reason must be between 10 and 200 characters"),
...
export const reviewFlaggedContentValidation = [
  param("flagId").isUUID().withMessage("Invalid flag ID format"),
  body("status")
